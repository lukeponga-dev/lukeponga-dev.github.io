...
window.addEventListener('scroll', function () {
        ...
        if (window.scrollY > 50) {
                navbar.style.backgroundColor = "rgba(30, 42, 71, 0.98)"; // Dark background
                navbar.style.padding = '10px 0'; // Reduced padding
                navbar.style.boxShadow = '0 2px 15px rgba(0, 0, 0, 0.2)'; // More prominent shadow
                navbarBrand.style.color = 'var(--light)'; // Light brand color
                navLinks.forEach(link => link.style.color = 'var(--light)'); // Light link colors
        } else {
                // Revert to original navbar style when at the top
                navbar.style.backgroundColor = "rgba(255, 255, 255, 0.95)"; // Light background
                navbar.style.padding = '20px 0'; // Original padding
                navbar.style.boxShadow = '0 2px 15px rgba(0, 0, 0, 0.1)'; // Original shadow
                navbarBrand.style.color = 'var(--primary)'; // Primary brand color
                navLinks.forEach(link => link.style.color = 'var(--dark)'); // Dark link colors
        }
});
